...
import { ChevronLeft, ChevronRight, TrendingUp, Clock, CheckCircle, AlertTriangle, AlertCircle } from "lucide-react";
...
export default function Receivables() {
  ...
  const { data, isLoading, error } = trpc.receivables.list.useQuery(queryInput);
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-1">
              ...
              <p className="text-2xl font-semibold text-blue-400">{Number(stats.openCount ?? 0).toLocaleString("pt-BR")}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-5 space-y-1">
              <div className="flex items-center gap-2 text-muted-foreground text-xs font-medium uppercase tracking-wider">
                <CheckCircle className="w-3.5 h-3.5" /> Recebidos
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  {["ID Externo", "Cliente", "Emissão", "Vencimento", "Valor Bruto", "Valor Pago", "Documento", "Status", "ERP"].map((h, i) => (
                    <th key={i} className={`px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider ${i >= 4 ? "text-right" : "text-left"}`}
                      style={i >= 4 && i <= 5 ? {} : {}}>{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={9} className="px-6 py-16 text-center text-muted-foreground">
                      ...
                      <p className="text-sm">Nenhum título a receber encontrado</p>
                  ...
                  data?.receivables.map((r) => (
                    <tr key={r.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                      <td className="px-6 py-4 font-mono text-xs text-muted-foreground">{r.externalId}</td>
                      <td className="px-6 py-4 font-medium text-foreground max-w-[180px] truncate">{r.customerName ?? "—"}</td>
                      <td className="px-6 py-4 text-muted-foreground">{formatDate(r.issueDate)}</td>
                      <td className="px-6 py-4 text-muted-foreground">{formatDate(r.dueDate)}</td>
                      <td className="px-6 py-4 text-right font-medium text-foreground">{formatCurrency(r.grossAmount)}</td>
                      <td className="px-6 py-4 text-right text-emerald-400">{r.paidAmount ? formatCurrency(r.paidAmount) : "—"}</td>
                      <td className="px-6 py-4 text-muted-foreground text-xs">{r.documentNumber || r.documentType || "—"}</td>
                      <td className="px-6 py-4"><StatusBadge status={r.status} /></td>
                      <td className="px-6 py-4"><ERPBadge erp={r.source} /></td>
                    </tr>
                  ))
                )}
              </tbody>
